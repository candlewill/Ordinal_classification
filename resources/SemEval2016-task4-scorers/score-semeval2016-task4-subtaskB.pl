...
foreach my $class ('positive', 'negative') {
    my $denomP = ($stats{'positive'}{$class} + $stats{'negative'}{$class}) > 0 ? ($stats{'positive'}{$class} + $stats{'negative'}{$class}) : 1;
    my $P = 100.0 * $stats{$class}{$class} / $denomP;

    my $denomR = (($stats{$class}{'positive'} + $stats{$class}{'negative'}) > 0) ? ($stats{$class}{'positive'} + $stats{$class}{'negative'}) : 1;
